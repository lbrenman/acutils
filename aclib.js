...
var _apiCentralBaseURL, _orgId, _accessToken;
...
function init(clientId, clientSecret, apiCentralBaseURL, orgId, callback) {
  consoleLog('init()')
  ...
  getToken(clientId, clientSecret, function(response) {
    if(response.success) {
      if(debugConsoleLog){console.log(response.data);}
      _accessToken = response.data.access_token;
      initCalled = true;
      if(callback){callback({success:true})}
    } else {
      if(debugConsoleLog){console.log('Token error: '+response.data);}
      if(callback){callback({success:false, data:'Token error: '+response.data})}
    }
  });
}

function getAxiosOptions(method, url, accessToken, orgId) {
